<x-form.index class="form-submit" action="{{ !empty($categoryBlog->id) ? route('manager.blog.category.update', ['id' => $categoryBlog->id, 'slug' => $categoryBlog->slug]) : route('manager.blog.category.create') }}" :method="!empty($categoryBlog->id) ? 'PUT' : 'POST'">
    <div>
